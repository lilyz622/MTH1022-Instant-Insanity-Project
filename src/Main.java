...
public class Main{
	...
	public void runMain(){
		...
		ArrayList<Pair[][]> finalSolutions = this.DisplayIntro(ui);
		...
		if (!finalSolutions.isEmpty()){
			...
			JOptionPane.showMessageDialog(null, "Restart Demo?", "THE FOUR CUBES",JOptionPane.OK_CANCEL_OPTION, cubepic);
			s.setVisible(false);
			s.dispose();
		} else {
			JOptionPane.showMessageDialog(null, "Restart Demo?", "THE FOUR CUBES",JOptionPane.OK_CANCEL_OPTION, cubepic);
	...
	public ArrayList<Pair[][]> DisplayIntro(JOptionPane ui) {
		ImageIcon cubepic = new ImageIcon("Cubes.jpg");
